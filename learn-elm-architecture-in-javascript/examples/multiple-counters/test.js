...
test('Test Update increment: update(1, "inc") returns 2', function(assert) {
  let action = createAction("inc", 0);
  var result = update({counters: [1] }, action);
...
test('Test negative state: update(-9, "inc") returns -8', function(assert) {
  let action = createAction("inc", 0);
  var result = update({counters: [-9] }, action);
...
test('Test reset counter when model/state is 6 returns 0', function(assert) {
  action = createAction("reset",0);
  var result = update({counters:[7]}, action);
  assert.equal(result.counters[0], 0);
});
...
test("createAction(INC, 0) will return {'inc', 0}", function(assert) {
  let INC = "inc";
  let action = createAction(INC, 0);
  assert.equal(action.type, 'inc');
